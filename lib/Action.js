...
/**     @member/Function run
    ...
    (submergence:Request) and wrap the result in a [Reply](submergence:Reply). Subject to immediate
    failure conditions if the [body](:Configuration#bodySchema) or [query]
    (:Configuration#querySchema) fail schema validation, if the agent is not [sufficiently
    authenticated](:Configuration#authentication), if the body's content-type is [not acceptable]
    (:Configuration#binaryStreams), or if the reaction Function synchronously throws an [Error]().
    Thrown Error conditions are routed through the templates by the http codes `403` and `406`.
@argument/submergence station
    The [submergence]() instance managing this request.
...
Action.prototype.run = function (station, agent, request, reply) {
    ...
    var remoteRequest = tech.request (options, function (response) {
        ...
        if (response.headers['x-substation-agent-status']) {
            ...
            agent[callName] (user, client, rememberMe, function (err) {
                if (err)
                    return reply.done (502);
                reply.latency ('update_auth');
                finalize();
            });
            return;
        }
...
@member/json authentication
    ...
    @member authentication#loggedIn
    @member authentication#domestic
...
@member/Boolean neverBuffer
    ...
    (submergence:Request.body) is always a [stream](stream.Readable).
