    ...
    DomainCollection.findOne ({ _id:request.params[0] }, function (err, domainRec) {
        ...
        if (request.body.confirmed && !domainRec.confirmed)
            return dns.resolveTxt (domainRec.requested, function (err, records) {
                ...
                var fullStr = 'sublayer.io='+domainRec._id;
                var found = false;
                ...
                if (request.body.active)
                    update.active = true;
                DomainCollection.update ({ _id:request.params[0] }, { $set:update }, function (err) {
                    if (err)
                        return reply.done ('502');
                    console.log (domainRec, domainRec.requested);
                    reply.event ('domainStatus', domainRec.requested, 'ready');
                    reply.done();
                });
            });
