...
function Gateway (parent, config, server) {
    ...
    function forwardEvent (domain, event, info, callback) {
        self.parent.getDomain (domain, function (domainRecord) {
            if (!domainRecord) {
                if (callback)
            ...
            var options = filth.clone (domainRecord.events[event].forward);
            ...
            request.write (JSON.stringify ({ serverEvent:event, info:info }));
            request.end();
        });
    }
