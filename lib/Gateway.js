...
/**     @submodule/class sublayer:Configuration
...
/**     @module/class sublayer:Gateway
    @root

@argument/sublayer parent
@argument/sublayer:Configuration
*/
function Gateway (parent, config, server) {
    ...
    function forwardEvent (domain, event, info, callback) {
        self.parent.getDomain (domain, function (err, domainRecord) {
            ...
            if (!domainRecord.config.events || !Object.hasOwnProperty.call (domainRecord.config.events, event)) {
                if (callback)
                    process.nextTick (callback);
                return;
            }
            var options = filth.clone (domainRecord.config.events[event]);
...
@callback
    @argument/sublayer:Action|undefined action
...
Gateway.prototype.getAction = function (request, pathstr, callback) {
    ...
    this.parent.getDomain (request.headers.host, function (err, domain) {
        if (err)
            return callback();
        if (!domain) {
            logger.warn ({ domain:request.headers.host }, 'unknown domain requested');
            return callback();
        }
        if (!domain.config.actions || !domain.config.actions.length)
            return callback();

        for (var i=0,j=domain.config.actions.length; i<j; i++) {
            var action = domain.config.actions[i];
